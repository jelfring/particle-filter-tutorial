...
# Particle filters
from core.particle_filters import ParticleFilterSIR, AuxiliaryParticleFilter
...
if __name__ == '__main__':
    ...
    print("Compare SIR with the auxiliary particle filter.")
    ...
    true_robot_motion_forward_std = 0.005  # 0.005
    true_robot_motion_turn_std = 0.002     # 0.002

    # Robot measurements are corrupted by measurement noise
    true_robot_meas_noise_distance_std = 0.5  # 0.2
    true_robot_meas_noise_angle_std = 0.10    # 0.05
    ...
    motion_model_forward_std = 0.50  # 0.10
    motion_model_turn_std = 0.10     # 0.02
    ...
    particle_filter_sir.initialize_particles_gaussian([robot.x, robot.y, robot.theta], [0.5, 0.5, 0.2])

    # Auxiliary particle filter (note only multinomial resampling is supported)
    particle_filter_aux = AuxiliaryParticleFilter(
        number_of_particles=number_of_particles,
        limits=pf_state_limits,
        process_noise=process_noise,
        measurement_noise=measurement_noise)
    particle_filter_aux.set_particles(particle_filter_sir.particles)
    ...
    for i in range(n_time_steps):
        ...
        # Update auxiliary particle filter
        particle_filter_aux.update(robot_forward_motion=robot_setpoint_motion_forward,
                                    robot_angular_motion=robot_setpoint_motion_turn,
                                    measurements=measurements,
                                    landmarks=world.landmarks)

        # Visualize particles after initialization (to avoid cluttered visualization)
        visualizer.draw_world(world, robot, particle_filter_sir.particles, hold_on=True, particle_color='g')
        visualizer.draw_world(world, robot, particle_filter_aux.particles, hold_on=True, particle_color='b')
