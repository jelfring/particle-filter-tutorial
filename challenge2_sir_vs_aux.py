...
if __name__ == '__main__':
    ...
    motion_model_forward_std = 0.5  # 0.10
    motion_model_turn_std = 0.20     # 0.02
    ...
    for trial in range(n_trials):
        ...
        for i in range(n_time_steps):
            ...
            e_aux = robot_pose - np.asarray(particle_filter_aux.get_average_state())

            errors_sir.append(np.linalg.norm(e_sir))
            errors_aux.append(np.linalg.norm(e_aux))

    print("SIR mean error: {}, std error: {}".format(np.mean(np.asarray(errors_sir)), np.std(np.asarray(errors_sir))))
    print("Aux mean error: {}, std error: {}".format(np.mean(np.asarray(errors_aux)), np.std(np.asarray(errors_aux))))
